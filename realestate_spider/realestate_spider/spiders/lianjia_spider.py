...
class LianjiaSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        """
        for url in start_url.lianjia_start_url:
    ...
    def parse_rent_old(self, response):
        house_list = response.xpath("//ul[@id='house-lst']/li")
        for house_item in house_list:
            try:
                rent_price = house_item.xpath("./div[@class='info-panel']/div[@class='col-3']/div[@class='price']/span[@class='num']/text()").extract()[0]
                url = house_item.xpath("./div[@class='pic-panel']/a/@href").extract()[0]
                title = house_item.xpath("./div[@class='info-panel']/h2/a/@title").extract()[0]
                community = "未知"
                try:
                    community = house_item.xpath("./div[@class='info-panel']/div[@class='col-1']/div[@class='where']/a/span/text()").extract()[0]
                except ValueError as ve:
                    self.log(ve.message, level=logging.WARN)
                    pass
                bedrooms = "0"
                try:
                    temp = house_item.xpath("./div[@class='info-panel']/div[@class='col-1']/div[@class='where']/span[@class='zone']/span/text()").extract()[0]
                    bedrooms = temp[:temp.find(u"室")]
                except ValueError as ve:
                    self.log(ve.message, level=logging.WARN)
                    pass
                area = ""
                try:
                    area_temp = house_item.xpath("./div[@class='info-panel']/div[@class='col-1']/div[@class='where']/span[@class='meters']/text()").extract()[0]
                    area = area_temp[:area_temp.find(u"平")]
                except ValueError as ve:
                    self.log(ve.message, level=logging.WARN)
                    pass
                ...
                orientation = "未知"
                try:
                    orientation = house_item.xpath("./div[@class='info-panel']/div[@class='col-1']/div[@class='where']/span[3]/text()").extract()[0]
                ...
                build_year = "未知"
                try:
                    content = house_item.xpath("./div[@class='info-panel']/div[@class='col-1']/div[@class='other']/div[@class='con']/text()").extract()
                    floor = content[0]
                    build_year = content[1]
                except IndexError as ie:
                    self.log(ie.message, level=logging.WARN)
                    pass
                ...
                renthouse_item["region"] = region.strip(u"\xc2\xa0")
                renthouse_item["community"] = community.strip(u"\xc2\xa0")
                renthouse_item["bedrooms"] = int(bedrooms)
                renthouse_item["title"] = title.strip(u"\xc2\xa0")
                renthouse_item["area"] = float(area)
        ...
        try:
            ...
            if cur_page_num == 1:
                for i in range(2, total_page_num + 1, 1):
                    ...
                    yield scrapy.Request(url=next_page, callback=self.parse_rent_old)
        except IndexError as ie:
            self.log(ie.message, level=logging.WARN)
    ...
    def parse_rent_detail(self, response):
        try:
            ...
            title = response.xpath("//div[@class='title-wrapper']/div[@class='content']/div[@class='title']/h1[@class='main']/text()").extract()[0]
            p_list = response.xpath("//div[@class='overview']/div[@class='content zf-content']/div[@class='zf-room']/p")
            area = 0
            bedrooms = 0
            floor = "未知"
            orientation = "未知"
            ...
            for p_item in p_list:
                try:
                    i_value = p_item.xpath("./i/text()").extract()[0]

                    if i_value == u"面积：":
                        area_temp = p_item.xpath("./text()").extract()[0]
                        area = float(area_temp[:area_temp.find(u"平")])
                    elif i_value == u"房屋户型：":
                        bedroom_temp = p_item.xpath("./text()").extract()[0]
                        bedrooms = int(bedroom_temp[:bedroom_temp.find(u"室")])
                    elif i_value == u"楼层：":
                        floor = p_item.xpath("./text()").extract()[0]
                    elif i_value == u"房屋朝向：":
                        orientation = p_item.xpath("./text()").extract()[0]
                    elif i_value == u"小区：":
                        community = p_item.xpath("./a/text()").extract()[0]
                    elif i_value == u"位置：":
                        region = p_item.xpath("./a/text()").extract()[0]
                except IndexError as ie:
                    self.log(ie.message, level=logging.WARN)
                    pass
                except ValueError as ve:
    ...
    def parse_rent(self, response):
        ...
        for house_item in house_list:
            try:
                detail_url = house_item.xpath("./div[@class='pic-panel']/a/@href").extract()[0]
